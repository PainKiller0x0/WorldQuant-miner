# --- Web仪表盘.py v5.6 (Add WQ Failed Status) ---
...
# --- v5.6: 新增加载/保存 failed_submissions 的函数 (带日志) ---
def load_failed_submissions():
    with failed_lock: # 使用新锁
        filepath = FAILED_SUBMISSIONS_FILE
        if not os.path.exists(filepath):
            logger.info(f"[Failed Load] File not found: {filepath}. Returning empty set.")
            return set()
        try:
            if os.path.getsize(filepath) < 2:
                logger.info(f"[Failed Load] File is empty or too small: {filepath}. Returning empty set.")
                return set()
            with open(filepath, 'r', encoding='utf-8') as f:
                data = json.load(f)
                if isinstance(data, (list, set)):
                    loaded_set = set(data)
                    sample = list(loaded_set)[:3]
                    logger.info(f"[Failed Load] Successfully loaded {len(loaded_set)} failed items from {filepath}. Sample: {sample}")
                    return loaded_set
                else:
                    logger.warning(f"[Failed Load] File {filepath} did not contain a list/set. Found type: {type(data)}. Returning empty set.")
                    return set()
        except json.JSONDecodeError as e:
            logger.error(f"[Failed Load] Error decoding JSON from {filepath}: {e}. Returning empty set.")
            return set()
        except IOError as e:
            logger.error(f"[Failed Load] IOError reading {filepath}: {e}. Returning empty set.")
            return set()
        except Exception as e:
            logger.error(f"[Failed Load] Unexpected error loading {filepath}: {e}", exc_info=True)
            return set()
...
def save_failed_submissions(failed_set):
    with failed_lock: # 使用新锁
        ...
        try:
            ...
            logger.info(f"[Failed Save] Successfully saved {len(failed_set)} items to {filepath}")
            return True
        except IOError as e:
            logger.error(f"[Failed Save] IOError saving {filepath}: {e}")
            return False
        except Exception as e:
            logger.error(f"[Failed Save] Unexpected error saving {filepath}: {e}", exc_info=True)
            return False
# --- v5.6 结束 ---
...
# --- v5.6: 优化 get_hopeful_alphas_stats (加入 is_failed_on_wq 标志) ---
def get_hopeful_alphas_stats():
    ...
    failed_set = load_failed_submissions() # <-- v5.6: 加载失败集合
        ...
        try:
            ...
            for alpha in valid_alphas_list:
                try:
                    ...
                    is_failed_on_wq = bool(expression in failed_set) # <-- v5.6: 新增失败标志

                    if is_submittable and not is_submitted and not is_failed_on_wq: # <-- v5.6: 失败的不计入待提交
                    ...
                    processed_alphas_temp.append({
                        ...
                        "turnover": perf_data.get('turnover', 0),
                        "returns": perf_data.get('returns', 0),
                        "checks": summary_str,
                        "timestamp": alpha.get('timestamp', 'N/A'),
                        "is_all_pass": is_all_pass,
                        "is_submittable": is_submittable,
                        "is_submitted": is_submitted,
                        "is_failed_on_wq": is_failed_on_wq, # <-- v5.6: 传递给前端
            ...
            def sort_key(alpha):
                ...
                # v5.6: 可以在这里加入 is_failed_on_wq 作为第四级排序 (False > True)
                # 让失败的排在更后面, 但暂时保持原样
...
def mark_alpha_failed():
    ...
    logger.info(f"[API /{operation.lower()}] Request received.")
    if not request.is_json:
        logger.warning(f"[API /{operation.lower()}] Request is not JSON.")
        return jsonify(status='error', message='Request must be JSON'), 400
    data = request.json
    expression = data.get('expression')
    if not expression or not isinstance(expression, str):
        logger.warning(f"[API /{operation.lower()}] Invalid or missing expression.")
        return jsonify(status='error', message='Invalid or missing expression'), 400

    logger.info(f"[API /{operation.lower()}] Received expression (first 50 chars): {expression[:50]}...")
    try:
        failed_set = load_failed_submissions() # 使用新函数加载
        original_size = len(failed_set)
        failed_set.add(expression)
        new_size = len(failed_set)

        if new_size > original_size:
            logger.info(f"[API /{operation.lower()}] Expression added to failed set. Attempting save.")
        else:
            logger.info(f"[API /{operation.lower()}] Expression already in failed set. Attempting save (idempotent).")

        if save_failed_submissions(failed_set): # 使用新函数保存
            logger.info(f"[API /{operation.lower()}] Operation successful for expression: {expression[:50]}...")
            return jsonify(status='success', message='WQ Failed status Marked')
        else:
            logger.error(f"[API /{operation.lower()}] Save operation failed for expression: {expression[:50]}...")
            return jsonify(status='error', message='Failed to save WQ Failed status'), 500
    except Exception as e:
        logger.critical(f"[API /{operation.lower()}] CRITICAL Error: {e}", exc_info=True)
        return jsonify(status='error', message=f'Internal server error while marking WQ Failed'), 500

@app.route('/api/unmark_failed_on_wq', methods=['POST'])
def unmark_alpha_failed():
    operation = "UnmarkFailed"
    logger.info(f"[API /{operation.lower()}] Request received.")
    if not request.is_json:
        logger.warning(f"[API /{operation.lower()}] Request is not JSON.")
        return jsonify(status='error', message='Request must be JSON'), 400
    ...
    try:
        failed_set = load_failed_submissions() # 使用新函数加载
        original_size = len(failed_set)
        failed_set.discard(expression)
        new_size = len(failed_set)

        if new_size < original_size:
            logger.info(f"[API /{operation.lower()}] Expression removed from failed set. Attempting save.")
        else:
            logger.info(f"[API /{operation.lower()}] Expression was not in failed set. Attempting save (idempotent).")

        if save_failed_submissions(failed_set): # 使用新函数保存
            logger.info(f"[API /{operation.lower()}] Operation successful for expression: {expression[:50]}...")
            return jsonify(status='success', message='WQ Failed status Unmarked')
        else:
            logger.error(f"[API /{operation.lower()}] Save operation failed for expression: {expression[:50]}...")
            return jsonify(status='error', message='Failed to save WQ Failed status'), 500
    except Exception as e:
        logger.critical(f"[API /{operation.lower()}] CRITICAL Error: {e}", exc_info=True)
        return jsonify(status='error', message=f'Internal server error while unmarking WQ Failed'), 500
# --- v5.6 结束 ---
