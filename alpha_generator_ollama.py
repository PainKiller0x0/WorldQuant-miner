# --- alpha_generator_ollama.py v13.3.0 (Producer Cache + Wildcard) ---
import argparse
import logging
import json
import os
...
# --- v13.3.0: 版本号 ---
CURRENT_GENERATOR_VERSION = "v13.3.0 (Producer Cache + Wildcard)"
# --- v13.3.0: 结束 ---
...
class AlphaGenerator:
    def __init__(self, wq: WorldQuant, api_config_path, batch_size=5, concurrency_level=2): 
        ...
        # --- v13.3.0: 生产者缓存 ---
        self.operators_cache = [] # 用于缓存 WQ 操作符
        self.operators_cache_expiry = 0 # 缓存过期时间
        self.OPERATOR_CACHE_DURATION_SECONDS = 600 # 10 分钟
        # --- v13.3.0: 结束 ---
            ...
            try:
                ...
                if isinstance(data, list):
                    ...
                    if new_set != self.submission_failures_set:
                        ...
                        if new_failures > 0: logger.info(f"[Failure Log] 成功加载并刷新失败日志。检测到 {new_failures} 个新失败案例，总计 {len(new_set)} 个。")
                        else: logger.info(f"[Failure Log] 成功加载失败日志。总计 {len(new_set)} 个 (无变化)。")
                        self.submission_failures_set = new_set
                        self.submission_failures_map = new_map
                else: logger.warning(f"[Failure Log] 文件 {filepath} 格式不正确（不是列表），已忽略。")
    ...
    def update_blacklist_count(self, identifier_name):
        with self.blacklist_lock:
            ...
            except IOError as e: logger.error(f"保存黑名单计数文件时出错: {e}")

    def is_using_blacklisted_identifier(self, alpha_code: str) -> bool:
        with self.blacklist_lock: current_counts = self.blacklist_counts
    ...
    def excavate_one_pearl(self, sample_size=200):
        ...
        with self.hopeful_file_lock: hopeful_expressions = {alpha.get('expression') for alpha in self.hopeful_alphas_cache}
        potential_pearls = []
            ...
            if isinstance(checks_list, list):
                for check in checks_list:
                    if isinstance(check, dict) and check.get('name') == 'Self-correlation':
                        self_corr_value = float(check.get('value', 0.0)); break
    ...
    # --- v13.3.0: 移除硬编码的外卡数量 ---
    def load_evolution_seeds(self, total_sample_size, wild_card_count):
        """
        v13.3.0: total_sample_size 和 wild_card_count 现在由 run() 方法从配置中传入。
        """
        seeds = []
                        ...
                        with open(self.hopeful_alphas_file, 'r', encoding='utf-8') as f:
                            ...
                            if content:
                                ...
                                if isinstance(loaded_data, list):
                                     self.hopeful_alphas_cache = loaded_data; seeds.extend(self.hopeful_alphas_cache)
                                else:
        ...
        # (v13.3.0: 70/30 的硬编码比例保留，但抽样数量已动态化)
        ...
        # (v13.3.0: wild_card_count 现在是动态的)
    ...
    def analyze_successful_patterns(self, top_k_pool=20, sample_size=7):
        ...
        selected_guidance = []; k = min(sample_size, len(operators))
        temp_ops = list(operators); temp_weights = list(weights)
        while len(selected_guidance) < k and temp_ops:
            ...
            try:
                ...
                temp_ops.pop(idx); temp_weights.pop(idx)
            except ValueError:
    ...
    # --- v13.2.0: 修复 save_hopeful_reports ---
    def save_hopeful_reports(self, new_hopeful_reports, max_pool_size=None):
        """ v13.2.0: max_pool_size 现在默认为 None。将从 system_config.json 动态读取。 """
        with self.hopeful_file_lock:
            ...
            for expr in unique_reports_map:
                if expr in self.submission_failures_set:
                    ...
                    expressions_to_archive.add(expr); continue
        ...
        while True:
            ...
            try:
                ...
                if isinstance(result, dict) and result.get("status") == "ERROR":
                    ...
                    if match:
                        ...
                        if error_type == "unknown variable":
                            should_blacklist = True; logger.warning(f"检测到无效变量: '{bad_identifier}'。WQ API 报告其未知。")
                            ...
                            if bad_identifier not in (self.fields if isinstance(self.fields, list) else []):
                                should_blacklist = True; logger.warning(f"检测到无效函数/操作符: '{bad_identifier}'。")
                            else: logger.info(f"Alpha 模拟出错: '{bad_identifier}' 是一个数据字段，但被误用为函数。已记录，不计入黑名单。")
                        if should_blacklist: self.update_blacklist_count(bad_identifier)
                    else: logger.warning(f"Alpha 模拟出错 (非特定标识符错误)，已记录: {idea_expr} | Error: {str(error_message)[:200]}...")
                    self.strategy_queue.task_done(); continue 

                if result in ["TIMEOUT"]:
                    log_report["status"] = result; self.log_tested_alphas([log_report])
                ...
                if isinstance(result, dict):
                    ...
                    self_corr_value = self._get_self_correlation(log_report); is_self_corr_ok = self_corr_value < 0.7
                    ...
                    if (is_high_quality or is_high_potential) and is_self_corr_ok:
                        if is_high_potential and not is_high_quality: logger.info(f"发现一个高潜力策略 (Fitness < 0, 但 Checks >= 5)，破格录用！ Fitness: {fitness_float:.3f}, Checks: {passed_count} PASS, Self-Corr: {self_corr_value:.3f}. Alpha: {idea_expr}")
                        else: logger.info(f"发现一个高质量策略！ Fitness: {fitness_float:.3f}, Checks: {passed_count} PASS, Self-Corr: {self_corr_value:.3f}. Alpha: {idea_expr}")
                        regular_code = result.get("regular", {}).get("code") if isinstance(result.get("regular"), dict) else None
                        hopeful_report = { "expression": regular_code or idea_expr, "alpha_id": alpha_id, "result_url": f"https://platform.worldquantbrain.com/alphas/regular/{alpha_id}", "grade": result.get("grade", "UNKNOWN"), "timestamp": log_report["timestamp"], "performance": log_report["performance"], "checks_summary": checks_summary }
                ...
                if isinstance(strategy, dict) and 'expression' in strategy:
                    try:
                        ...
                        log_report["status"] = "EXCEPTION_WORKER"; self.log_tested_alphas([log_report])
                    except Exception as log_exc: logger.critical(f"在异常处理中再次发生错误，无法记录: {log_exc}")
            finally:
                self.strategy_queue.task_done()


    # --- v13.3.0: 重构 run 循环, 缓存 Operators ---
    def run(self, mode='discover'):
        ...
        # (v13.3.0: 移除在循环外获取 operators 和 fields 的逻辑)
        ...
        while True:
            try:
                ...
                if self.strategy_queue.qsize() >= self.queue_max_size:
                    ...
                    continue # 返回循环顶部，重新检查队列
                # --- v13.3.0: 修复结束 ---

                # 2. 检查 WQ 字段 (本地调用, 成本低)
                self.fields = self.wq.get_data_fields()
                if not self.fields:
                     logger.error("[生产者] 无法获取字段，将在60秒后重试。")
                     time.sleep(60)
                     continue
                     
                # 3. (新) 检查并刷新 WQ 操作符缓存
                now = time.time()
                if now > self.operators_cache_expiry:
                    logger.info("[生产者] WQ operator 缓存已过期，正在刷新...")
                    self.operators_cache = self.wq.get_operators() # API 调用
                    
                    if self.operators_cache == "RATE_LIMIT":
                         logger.critical("[生产者] 获取操作符时遭遇 WorldQuant 429。看门狗 B 将在下次调用时处理。")
                         self.operators_cache_expiry = now + 60 # 60秒后重试
                         continue
                    if not self.operators_cache:
                         logger.error("[生产者] 无法获取操作符，将在60秒后重试。")
                         time.sleep(60)
                         continue
                         
                    # 缓存成功
                    self.operators_cache_expiry = now + self.OPERATOR_CACHE_DURATION_SECONDS
                    logger.info(f"[生产者] Operator 缓存已刷新 ({len(self.operators_cache)} 个)，将在 {self.OPERATOR_CACHE_DURATION_SECONDS/60:.0f} 分钟后到期。")
                
                # (v13.3.0: 确保 self.operators 被设置为缓存)
                self.operators = self.operators_cache
                # --- v13.3.0: 缓存逻辑结束 ---
                ...
                # --- v13.3.0: 动态外卡池 ---
                if mode == 'evolve':
                    # (v13.3.0: 从 config 读取外卡数量)
                    config = load_system_config()
                    wild_card_count_from_config = config.get("evolver_wildcard_count", 5) # 默认 5
                    
                    evolution_seeds = self.load_evolution_seeds(
                        total_sample_size=20, # (这个暂时还是硬编码)
                        wild_card_count=wild_card_count_from_config # (这个是动态的)
                    )
                ...
                if isinstance(idea, dict) and idea.get("expression"):
                    ...
                    with self.tested_alphas_lock: is_tested = expr in self.tested_alphas
                    is_known_failure = expr in self.submission_failures_set
                    if is_tested:
                        logger.info(f"[生产者] 策略 {expr[:60]}... 已被测试过，丢弃。"); continue
                    if is_known_failure:
                        logger.warning(f"[生产者] 策略 {expr[:60]}... 已在'失败日志'中，丢弃。"); continue
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Alpha Generator v13.3.0 (Producer Cache + Wildcard)') # v13.3
    ...
    parser.add_argument('--batch-size', type=int, default=5, help="(已弃用)")
    ...
    while wq_client is None:
        try:
            ...
            if retry_count <= MAX_INIT_RETRIES:
                ...
                time.sleep(LONG_SLEEP); retry_count = 0
