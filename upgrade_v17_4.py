...
# --- utils.py v17.2 (增加 Evolver 采样配置) ---
CONTENT_UTILS = r'''# --- utils.py v17.2 (Config: Evolver Tuning) ---
import logging
...
CONTENT_GENERATOR = r'''# --- alpha_generator_ollama.py v13.3.16 (Dynamic Sampling) ---
import argparse
import logging
import json
import os
...
def main():
    print("开始升级到 v17.4.0 (Evolver 视野拓展)...")
    
    # 更新文件
    update_file("utils.py", CONTENT_UTILS)
    update_file("alpha_generator_ollama.py", CONTENT_GENERATOR)
    
    # 删除配置文件以触发新默认值
    if os.path.exists("system_config.json"):
        try:
            # 如果是文件直接删
            if os.path.isfile("system_config.json"):
                os.remove("system_config.json")
            # 如果是目录（之前的bug），用 shutil.rmtree
            elif os.path.isdir("system_config.json"):
                shutil.rmtree("system_config.json")
                # 立即创建一个空文件，防止 Docker 再次建目录
                with open("system_config.json", 'w') as f: f.write("{}")
            print("🗑️  已重置 system_config.json 以加载新参数")
        except Exception as e:
            print(f"⚠️ 重置配置文件失败: {e}")
